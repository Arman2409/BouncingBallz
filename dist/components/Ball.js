...
class Ball {
    ...
    constructor(x, y, ballRadius, ctx) {
        ...
        if (canvasHeight - (y + ballRadius) > 0)
            fall();
        else
            stop();
    }
    ...
    isNearBorder = () => {
        ...
        if (x <= radius)
            return "left";
        if (x >= surfaceWidth - radius)
            return "right";
        return false;
    };
    draw = (type, ellipseX, ellipseY, extraY, gradient) => {
        const { x, y, radius, rotation, colors, ctx } = { ...this };
        if (type === "circle")
            ctx.arc(x, y, radius, 0, Math.PI * 2, false);
        if (type === "ellipse")
            ctx.ellipse(x, extraY ? y + extraY : y, ellipseX, ellipseY, 0, 0, Math.PI * 2);
    ...
    handleFalling = () => {
        const { y, radius, side, surfaceHeight, draw, bounce } = { ...this };
    ...
    handleBouncing = () => {
        ...
        if (y <= surfaceHeight - bounceHeight)
            fall();
        // checking if the ball is going towards the border 
        if (isNearBorder() === side)
            fall();
        draw("circle");
    };
    fall = () => {
